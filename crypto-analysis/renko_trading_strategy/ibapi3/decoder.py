...
class Decoder(Object):
    ...
    def processExecutionDataMsg(self, fields):
        ...
        execu = ibpy.execution.Execution()
        execu.orderId = orderId
        execu.execId = decode(str, fields)
        execu.time = decode(str, fields)
        execu.acctNumber = decode(str, fields)
        execu.exchange = decode(str, fields)
        execu.side = decode(str, fields)

        if self.serverVersion >= MIN_SERVER_VER_FRACTIONAL_POSITIONS:
                execu.shares = decode(float, fields)
        else:
                execu.shares = decode(int, fields)

        execu.price = decode(float, fields)
        execu.permId = decode(int, fields) # ver 2 field
        execu.clientId = decode(int, fields)  # ver 3 field
        execu.liquidation = decode(int, fields) # ver 4 field

        if version >= 6:
            execu.cumQty = decode(float, fields)
            execu.avgPrice = decode(float, fields)

        if version >= 8:
            execu.orderRef = decode(str, fields)

        if version >= 9:
            execu.evRule = decode(str, fields)
            execu.evMultiplier = decode(float, fields)
        if self.serverVersion >= MIN_SERVER_VER_MODELS_SUPPORT:
            execu.modelCode = decode(str, fields)
        if self.serverVersion >= MIN_SERVER_VER_LAST_LIQUIDITY:
            execu.lastLiquidity = decode(int, fields)

        self.wrapper.execDetails(reqId, contract, execu)
    ...
    def readLastTradeDate(self, fields, contract, isBond):
