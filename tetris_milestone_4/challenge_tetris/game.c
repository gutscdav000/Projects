...
highscore_t *game(highscore_t *highscores) {
  ...
  while(1) {
    ...
    case INIT:               // Initialize the game, only run one time 
      ...
      //state = ADD_PIECE;
      state = ADD_PIECE;
      obstruction = create_obstruction (w->upper_left_x - 15, w->upper_left_y + 35);
      break;
    case SPLASH_SCREEN:
      initscr();
      //enable color
	...
	if (arrow == UP) {
	  ...
	  else if(c == '+' && w->width < MAX_WELL_WIDTH) {
	    undraw_well(w);
	    w->width++;
	    draw_well(w);
	    if(check_collision(current) == MOVE_FAILED) {
	  ...
	  else if(c == '-' && w->width > MIN_WELL_WIDTH) {
	    undraw_well(w);
	    w->width--;
	    draw_well(w);
	    if(check_collision(current) == MOVE_FAILED) {
...
tetromino_t *create_obstruction (int initial_x, int initial_y) {

  tetromino_t *tet = malloc(sizeof(tetromino_t));
  memcpy(tet, &tetromino_types[7], sizeof(tetromino_t));
